...
# ─── RESEND API CONFIG ──────────────────────────────────────────
resend.api_key = os.getenv('RESEND_API_KEY')  # NEVER hardcode API keys!
...
def contact():
    try:
        ...
        if not all([name, email, domain, message]):
            return jsonify({'success': False, 'message': 'All fields required.'}), 400

        print(f"📬 Contact → {name} ({email}) - {domain}")

        # Send email using Resend API
        params = {
            "from": "Crypto AI <onboarding@resend.dev>",
            "to": [RECIPIENT_EMAIL],
            "reply_to": email,
            "subject": f"[Crypto AI] {domain} - {name}",
            "html": f"""
            <html>
        ...
        return jsonify({'success': False, 'message': 'Error occurred. Please try again.'}), 500
